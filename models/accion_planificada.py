...
class accion_planificada (models.Model):
    ...
    def listado_facturas(self):
        ...
        facturas_ids = self.search([('payment_state', '=', 'paid')])
        facturas_no_pagadas_ids = self.search([('move_type', '=', 'out_invoice'),('payment_state', '=', 'not_paid')])
        if facturas_ids:
            listado = "<br/>Listado de facturas pagadas:"
            ...
            listado += "<br/>Listado de facturas no pagadas:"
            for rexistro in facturas_no_pagadas_ids:
                listado = listado + "<br/>" + str(rexistro.partner_id.display_name) + "-> " + str(rexistro.amount_total)
